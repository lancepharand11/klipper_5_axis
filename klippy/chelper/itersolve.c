...
//
// Copyright (C) 2018-2020  Kevin O'Connor
//
                          ...
                          double abs_start, double abs_end)
{
    ...
    struct timepos old_guess = { start, sk->commanded_pos };
    struct timepos guess = old_guess;
    ...
    for (;;) {
        ...
        if (!(next_time > low_time && next_time < high_time)) { // or NaN
            // Next guess is outside bounds - validate it
            if (have_bracket) {
                // Fall back to bisection
                next_time = (low_time + high_time) * 0.5;
                check_oscillate = 0;
            } else if (guess.time >= end) {
                // No more steps in requested time range
                break;
            } else {
                // Exponential search
                next_time = high_time;
        ...
        if (fabs(guess_dist) > 1.e-9) {
            ...
            if (rel_dist > 0.) {
                ...
                have_bracket = 1;
            } else if (rel_dist < -(2. * half_step + 1.e-8)) {
                // Found direction change
                sdir = !sdir;
                target = sdir ? (target + 2. * half_step) 
                              : (target - 2. * half_step);
                low_time = last_time;
                high_time = guess.time;
            ...
            if (!have_bracket || (high_time - low_time > 1.e-9)) {
        ...
        if (ret)
            return ret;
        target = sdir ? (target + 2. * half_step) : (target - 2. * half_step);

        // Reset
        double seek_time_delta = 1.5 * (guess.time - last_time);
        if (seek_time_delta < 1.e-9)
            seek_time_delta = 1.e-9;
...
check_active(struct stepper_kinematics *sk, struct move *m)
{
    ...
    // Check for linear axes
    if ((af & AF_X) && m->axes_r.x != 0.)
        return 1;
    if ((af & AF_Y) && m->axes_r.y != 0.)
        return 1;
    if ((af & AF_Z) && m->axes_r.z != 0.)
        return 1;
    
    // Check for rotational axes
    if ((af & AF_U) && m->axes_r.u != 0.)
        return 1;
    if ((af & AF_W) && m->axes_r.w != 0.)
        return 1;

    return 0;
}

// Generate step times for a range of moves in the trapq
int32_t __visible
itersolve_generate_steps(struct stepper_kinematics *sk, double flush_time)
{
    ...
    // Start from the first move not fully behind last_flush_time
    ...
    for (;;) {
        ...
        if (check_active(sk, m)) {
            if (skip_count && sk->gen_steps_pre_active) {
                ...
                do {
                    int32_t ret = itersolve_gen_steps_range(sk, pm,
                                                            abs_start,
                                                            flush_time);
                    if (ret)
                        return ret;
            ...
            int32_t ret = itersolve_gen_steps_range(sk, m,
                                                    last_flush_time,
                                                    flush_time);
            if (ret)
                return ret;
            ...
            // Move doesn't affect this stepper
...
// Check if this stepper is likely to be active within the flush_time range
double __visible
...
itersolve_is_active_axis(struct stepper_kinematics *sk, char axis)
{
    switch (axis) {
    case 'x':
        return (sk->active_flags & AF_X) != 0;
    case 'y':
        return (sk->active_flags & AF_Y) != 0;
    case 'z':
        return (sk->active_flags & AF_Z) != 0;
    case 'u':
        return (sk->active_flags & AF_U) != 0;
    case 'w':
        return (sk->active_flags & AF_W) != 0;
    }
    return 0;
}

// Set or replace the trapezoid queue pointer
void __visible
                                   ...
                                   double u, double w)
{
    ...
    m.start_pos.u = u;
    m.start_pos.w = w;
    // Arbitrary 1-second move, sample at 0.5s
    m.move_t = 1.0;
    return sk->calc_position_cb(sk, &m, 0.5);
}
